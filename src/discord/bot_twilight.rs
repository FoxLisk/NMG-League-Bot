...
mod state {
    ...
    impl State {
        pub(super) fn new(cache: InMemoryCache, client: Client, aid: Id<ApplicationMarker>, pool: SqlitePool) -> Self {
        ...
        pub(super) async fn get_private_channel(&self, user: Id<UserMarker>) -> Result<Id<ChannelMarker>, String> {
            if let Some(id) = self.private_channels.get(&user) {
                return Ok(id.clone())
            }

            let created = self.client.create_private_channel(user.clone()).exec().await.map_err(|e| e.to_string())?;
            if created.status().is_success() {
                let chan = created.model().await.map_err(|e| e.to_string())?;
                self.private_channels.insert(user, chan.id.clone());
                Ok(chan.id)
            } else {
                Err(format!("Error result creating private channel: {}", created.status()))
            }
        }
...
fn can_create_race(
    ...
    let role = get_admin_role(guild_id, state).ok_or("Error: Cannot find admin role".to_string())?;
    let member = state.cache.member(guild_id, user_id).ok_or("Error: cannot find member".to_string())?.value().clone();
...
fn button_component<S1: Into<String>, S2: Into<String>>(label: S1, custom_id: S2, style: ButtonStyle) -> Component {
    Component::Button(
        Button {
...
If anything goes wrong, tell an admin there was an issue with race `{}`",
    ...
    let resp = state.client.create_message(dm)
        .components(
          &[
              Component::ActionRow(
                  ActionRow {
                      components: vec![
                          button_component("Start run", CUSTOM_ID_START_RUN, ButtonStyle::Primary)
                      ]
                  }
              )
          ]
...
async fn handle_create_race(mut ac: Box<ApplicationCommand>, state: &Arc<State>) -> Result<InteractionResponse, String> {
    let gid = ac.guild_id.ok_or("Create race called outside of guild context".to_string())?;
    let uid = ac.author_id().ok_or("Create race called by no one ????".to_string())?;
    ...
    if ac.data.options.len() != 2 {
        return Err(format!("Expected exactly 2 options for {}", CREATE_RACE_CMD));
    }
    ...
    if p1 == p2 {
        return Ok(plain_interaction_response("The racers must be different users"));
    }


    let new_race = NewRace::new();
    let race = new_race.save(&state.pool).await.map_err(|e| format!("Error saving race: {}", e))?;
    let (r1, r2) = race.select_racers(p1, p2, &state.pool).await.map_err(|e| format!("Error saving race: {}", e))?;
    ...
    if let Err(err) = first.and(second) {
        return Ok(plain_interaction_response(format!("Internal error creating race: {}", err)));
    }
    Ok(plain_interaction_response(
      format!(
        ...
        Interaction::ApplicationCommand(ac) => {
            ...
            match ac.data.name.as_str() {
                CREATE_RACE_CMD => {
                    ...
                    if let Err(e) = state.interaction_client().create_response(
                        interaction_id,
                        &token,
                        &resp
                    ).exec().await {
                ...
                _ => {
                    println!("Twilight bot: Unhandled ApplicationCommand: {}", ac.data.name);
