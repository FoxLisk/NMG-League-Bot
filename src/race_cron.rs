...
fn format_secs(secs: u64) -> String {
    ...
    let hours = mins / 60;
    if hours > 0 {
        format!("{hours}:{mins:02}:{secs:02}", hours=hours, mins=mins % 60, secs=secs % 60 % 60)
    } else {
        format!("{mins:02}:{secs:02}", mins=mins % 60, secs=secs % 60 % 60)
    }
}
        ...
        RaceRunState::VOD_SUBMITTED => {
            let bot_time = match (run.run_started, run.run_finished) {
                (Some(start), Some(finish)) => {
                    format_secs((finish - start) as u64)
                },
                _ => "N/A".to_string()
            };
...
async fn sweep(pool: &SqlitePool) {
    ...
    let webhook = http_client.get_webhook_from_url(&dotenv::var(WEBHOOK_VAR).unwrap()).await.unwrap();
    println!("Sweep...");
    let active_races = match Race::active_races(pool).await {
        Ok(rs) => rs,
        Err(e) => {
            println!(
                "Error fetching active races: {}", e);
                return;
            }
    };
