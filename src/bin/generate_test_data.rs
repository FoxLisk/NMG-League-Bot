...
fn get_players(start: i32, add_me: bool) -> Vec<NewPlayer> {
    ...
    for i in start..end {
        ...
        let np = NewPlayer::new(name, discord_id, Some(racetime_username), Some(twitch_id), true);
        players.push(np);
    }

    if add_me {
        let me = NewPlayer::new("FoxLisk", "255676979460702210", Some("FoxLisk#8582"), Some("foxlisk"), true);
        let me_alt = NewPlayer::new("Me Test", "1031811909223206912", Some("NA#1234"), Some("foxtest69"), true);
        players.push(me);
        players.push(me_alt);
    }
    players
