...
fn generate_next_round_pairings_swiss(bracket: &Bracket,
...
fn generate_next_round_pairings_round_robin(bracket: &Bracket,
                                      conn: &mut SqliteConnection,
) -> Result<(), BracketError> {
    let rounds = bracket.rounds(conn)?;
    let mut highest_round_num = 0;
...
fn generate_next_round_pairings(
    ...
    match bracket.bracket_type()? {
        BracketType::Swiss => {
            generate_next_round_pairings_swiss(bracket, conn)
        }
        BracketType::RoundRobin => {
            generate_next_round_pairings_round_robin(bracket, conn)
        }
    }
}

fn generate_initial_pairings_swiss(bracket: &mut Bracket, conn: &mut SqliteConnection) -> Result<(), BracketError> {
...
fn polygon_indices(n: usize, rotation: usize) -> Option<(Vec<(usize, usize)>, usize)> {
    ...
    /*
        for even numbers we imagine a polygon of N-1 sides with a side on the bottom. We number
        clockwise from the bottom left vertex, with N marked in the middle
                2
               / \
            1 | 5 |3
            0 |___|4
        ...
        players play the person across from them on the polygon, with the leftout player at the top
...
fn generate_pairings_round_robin(bracket: &Bracket, round: &BracketRound, conn: &mut SqliteConnection) -> Result<(), BracketError> {
    let mut players = bracket.players(conn)?;
    players.sort_unstable_by_key(|p| p.id);
    let pairings = polygon_method(&players, round.round_num as usize).ok_or(BracketError::RoundRobinError("Unable to generate pairings".to_string()))?;
    let mut nbrs = vec![];
...
fn generate_initial_pairings_round_robin(bracket: &mut Bracket, conn: &mut SqliteConnection) -> Result<(), BracketError> {
    let new_round = NewBracketRound::new(bracket, 1);
    let round = new_round.save(conn)?;
    generate_pairings_round_robin(bracket, &round, conn)?;
...
impl NewBracket {
    pub fn new<S: Into<String>>(season: &Season, name: S, type_: BracketType) -> Self {
        ...
        let state = serde_json::to_string(&BracketState::Unstarted).unwrap_or("Unknown".to_string());
        Self {
            season_id: season.id,
            name: name.into(),
            state,
...
#[cfg(test)]
mod tests {
    use crate::models::brackets::{BracketState, polygon_method};
    ...
    fn test_polygon_method_four() {
        ...
        let r1_pairs = r1.iter().map(|(pp1, pp2)| (pp1.id, pp2.id)).collect::<Vec<_>>();
        let r2_pairs = r2.iter().map(|(pp1, pp2)| (pp1.id, pp2.id)).collect::<Vec<_>>();
        let r3_pairs = r3.iter().map(|(pp1, pp2)| (pp1.id, pp2.id)).collect::<Vec<_>>();
    ...
    fn test_polygon_method_six() {
        ...
        let rounds = (1..=5)
            .map(|r|
                polygon_method(&players, r).unwrap()
                    .iter().map(|(p1, p2)| (p1.id, p2.id)).collect::<Vec<_>>()
            )
            .collect::<Vec<_>>();
        assert_eq!(
            vec![
